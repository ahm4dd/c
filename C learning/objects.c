...
snek_object_t *snek_add(snek_object_t *a, snek_object_t *b) {
  ...
  if (a->kind == INTEGER) {
    ...
    if (b->kind != STRING) {
      ...
      size_t new_size = strlen(a->data.v_string) + strlen(b->data.v_string) + 1;
      char *temp_string = calloc(new_size, sizeof(char));
    ...
    if (b->kind != ARRAY) {
      ...
      for (int i = 0; i < b->data.v_array.size; ++i) {
        snek_array_set(new_arr, a->data.v_array.size+i, snek_array_get(b, i));
      }
      return new_arr;
    }
  }
